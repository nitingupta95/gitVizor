...
import { aiSummariseCommit } from "@/lib/ai";
...
export const getCommitHashes = async (githubUrl: string, githubToken?: string): Promise<Response[]> => {
  ...
  const actualToken = await getGithubToken(githubToken);
  const octokit = new Octokit({ auth: actualToken });
...
export const pollCommits = async (projectId: string) => {
  ...
  const actualToken = await getGithubToken(githubToken);
  const commitHashes = await getCommitHashes(githubUrl, actualToken);
  ...
  const summaryResponse = await Promise.allSettled(unpocessedCommits.map(commit => {
    return summariseCommit(githubUrl, commit.commitHash, actualToken);
  }));
...
async function filterUnprocessedCommits(projectId: string, commitHashes: Response[]) {
  ...
  const unpocessedCommits = commitHashes.filter(
